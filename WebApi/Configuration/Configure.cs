    ...
    public static class Configure
    {
        ...
        public static async void AddUsers(this WebApplication app)
        {
            using (var scope = app.Services.CreateScope())
            {
                ...
                var find = await userManager.FindByEmailAsync("wielki@chlop.pl");
                if (find == null)
                {
                    UserEntity user = new UserEntity() { Email = "wielki@chlop.pl", UserName = "WielkiChlop" };
                    var saved = await userManager?.CreateAsync(user, "Kochampieski123@");
                    if (saved.Succeeded)
                    {
